...
class VideoClsDataset(Dataset):
    ...
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
                 ...
                 num_segment=1, num_crop=1, test_num_segment=10, test_num_crop=3, args=None, ds_size=None):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            if self.ds_size != None:
                index = np.random.choice(self.dataset_samples)
...
class VideoDistillation(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        if self.ds_size is not None:
            index = np.random.choice(np.arange(len(self.clips)))
        ...
        if self.num_sample > 1:
            ...
            for idx in range(self.num_sample):
                process_data_0, process_data_1, mask = self.transform((images[idx * self.new_length : (idx+1) * self.new_length], None))  # T*C,H,W
                ...
                data_list.append(process_data_0)
                teacher_data_list.append(process_data_1)
                mask_list.append(mask)
            return data_list, teacher_data_list, mask_list
        else:
...
def get_random_sampling_rate(max_sampling_rate, min_sampling_rate):
    ...
    if max_sampling_rate > 0:
        ...
        return np.random.randint(min_sampling_rate, max_sampling_rate) if min_sampling_rate < max_sampling_rate else max_sampling_rate
    else:
