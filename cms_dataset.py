...
class VisionPropDataset(Dataset):
    ...
    def __init__(self, root_path, config, transforms, mode='train', debug=False):
        ...
        self.rollout_ts = []
        self.prop_latent = []
        self.setup_data_pipeline(transforms)
        if mode == 'deploy':
            return
        self.num_samples = 0
        ...
        # assert self.config.lookhead[0] > 0.0, "Lookhead should be larger than 0.0 for training"
    ...
    def process_raw_latent(self, data, ts):
        ...
        # Latent smoothing
        for i in range(data.shape[1]):
            time_shifted_data = np.zeros_like(data[:,i])
            for k in range(len(lookheads)):
                ...
                time_shifted_data = np.roll(data[:,i],
                                      int(-current_lookhead))
                if current_lookhead >= 0:
                    time_shifted_data[-current_lookhead:] = data[-current_lookhead:, i]
                else:
                    time_shifted_data[:-current_lookhead] = data[:-current_lookhead, i]
                predictive_latent.append(np.expand_dims(time_shifted_data,1))
    ...
    def setup_data_pipeline(self, transforms):
        if self.config.input_use_depth:
            ...
            self.preprocess_pipeline = transforms
    ...
    def __getitem__(self, idx):
        ...
        if self.config.input_use_imgs > 0:
            frame_idx_start = int(self.inputs[idx][-1].numpy())
            imgs = []
            for i in reversed(range(self.config.input_use_imgs)):
            ...
            if self.config.grayscale:
               imgs = rgb_to_grayscale(imgs, num_output_channels=1).squeeze(1)
