...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, save_numbered=False, obj_int=False):
    ...
    if loss_scaler is not None:
        ...
        for checkpoint_path in checkpoint_paths:
            ...
            if obj_int:
                save_on_master(to_save, curr_ckpt_path)
            else:
...
def remove_key_in_checkpoint(checkpoint, remove_key_list=None):
    if isinstance(remove_key_list, list):
        original_key_list = list(checkpoint.keys())
        for k in original_key_list:
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, obj_int=False):
    output_dir = Path(args.output_dir)
    if loss_scaler is not None:
        # torch.amp
        ...
        if args.resume:
            ...
            if 'optimizer' in checkpoint and 'epoch' in checkpoint and type(checkpoint['epoch']) is int and optimizer is not None:
        ...
        if args.eval and args.resume_best:
            print("Auto resume checkpoint: best")
...
def multiple_pretrain_samples_collate(batch, fold=False):
    ...
    Returns:
        (tuple): collated data batch.
    """
    inputs_0, inputs_1, masks = zip(*batch)
    inputs_0 = [item for sublist in inputs_0 for item in sublist]
    inputs_1 = [item for sublist in inputs_1 for item in sublist]
    masks = [item for sublist in masks for item in sublist]
    inputs_0, inputs_1, masks = (
        default_collate(inputs_0),
        default_collate(inputs_1),
        default_collate(masks),
    )
    if fold:
        return [inputs_0], [inputs_1], masks
    else:
        return inputs_0, inputs_1, masks
